{{config(
  alias = 'balancer_v3_pools_gnosis'
...
WITH token_data AS (
    ...
    pools AS (
      ...
      SELECT
        ...
        'stable' AS pool_type
      FROM token_data c
      INNER JOIN {{ source('balancer_v3_gnosis', 'StableSurgePoolFactory_call_create') }} cc
        ON c.pool = cc.output_pool
      CROSS JOIN UNNEST(c.tokens) AS t(tokens)
      ...
      SELECT
        ...
        'ECLP' AS pool_type
      FROM token_data c
      INNER JOIN {{ source('balancer_v3_gnosis', 'GyroECLPPoolFactory_call_create') }} cc
          ON c.pool = cc.output_pool
      CROSS JOIN UNNEST(c.tokens) AS t(tokens)
    ) zip 
          ),
...
SELECT 
  ...
  now() AS updated_at,
  'balancer_v3_pools_gnosis' AS model_name,
