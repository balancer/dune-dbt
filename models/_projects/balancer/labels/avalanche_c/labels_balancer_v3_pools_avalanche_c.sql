{{config(
  alias = 'labels_v3_pools_avalanche_c'
  , materialized = 'view'
...
WITH token_data AS (
    ...
    FROM (
        ...
        FROM {{ source('balancer_v3_avalanche_c', 'Vault_evt_PoolRegistered') }}
    ) AS pool_data
...
pools AS (
    ...
    FROM (
        ...
        FROM token_data c
        INNER JOIN {{ source('balancer_v3_avalanche_c', 'WeightedPoolFactory_call_create') }} cc
        ...
        FROM token_data c
        INNER JOIN {{ source('balancer_v3_avalanche_c', 'ReclammPoolFactory_call_create') }} cc
            ON c.pool = cc.output_pool
        ...
        SELECT
            ...
            'stable' AS pool_type
        FROM token_data c
        INNER JOIN {{ source('balancer_v3_avalanche_c', 'StablePoolFactory_call_create') }} cc
            ON c.pool = cc.output_pool
        CROSS JOIN UNNEST(c.tokens) AS t(tokens)
        ...
        SELECT
            ...
            'stable' AS pool_type
        FROM token_data c
        INNER JOIN {{ source('balancer_v3_avalanche_c', 'StableSurgePoolFactory_call_create') }} cc
            ON c.pool = cc.output_pool
        CROSS JOIN UNNEST(c.tokens) AS t(tokens)
        ...
        SELECT
            ...
            'LBP' AS pool_type
        FROM token_data c
        INNER JOIN {{ source('balancer_v3_avalanche_c', 'LBPoolFactory_call_create') }} cc
            ON c.pool = cc.output_pool
        CROSS JOIN UNNEST(c.tokens) AS t(tokens)
    ) zip
),
...
SELECT
    ...
    CASE
        WHEN pool_type IN ('stable', 'LBP', 'ECLP') THEN lower(pool_symbol)
        ELSE lower(concat(array_join(array_agg(token_symbol ORDER BY token_symbol), '/'), ' ', array_join(array_agg(cast(norm_weight AS varchar) ORDER BY token_symbol), '/')))
    END AS name,
    pool_type,
    ...
    now() AS updated_at,
    'balancer_v3_pools_avalanche_c' AS model_name,
