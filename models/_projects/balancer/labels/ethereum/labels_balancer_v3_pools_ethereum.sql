...
WITH token_data AS (
    ...
    pools AS (
      ...
      SELECT
        ...
        'stable' AS pool_type
      FROM token_data c
      INNER JOIN {{ source('balancer_v3_ethereum', 'StableSurgePoolFactory_call_create') }} cc
        ON c.pool = cc.output_pool
      CROSS JOIN UNNEST(c.tokens) AS t(tokens)
      ...
      SELECT
        ...
        'LBP' AS pool_type
      FROM token_data c
      INNER JOIN {{ source('balancer_v3_ethereum', 'LBPoolFactory_call_create') }} cc
        ON c.pool = cc.output_pool
      CROSS JOIN UNNEST(c.tokens) AS t(tokens)
      ...
      SELECT
        ...
        'ECLP' AS pool_type
      FROM token_data c
      INNER JOIN {{ source('balancer_v3_ethereum', 'GyroECLPPoolFactory_call_create') }} cc
          ON c.pool = cc.output_pool
      CROSS JOIN UNNEST(c.tokens) AS t(tokens)
    ) zip 
          ),
