{{
    config(
        alias = 'v3_sonic_erc4626_token_mapping',
        materialized = 'table'
    )
}}
...
WITH vault_mappings AS (
    SELECT DISTINCT
        ...
        amountUnderlying
    FROM {{ source('beethoven_x_v3_sonic', 'Vault_evt_LiquidityAddedToBuffer') }} AS b
...
underlying_tokens AS (
    ...
    JOIN {{ source('erc20_sonic', 'evt_Transfer') }} t
